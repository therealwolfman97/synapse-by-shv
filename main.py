...
class SynapseApp(App, StorageMixin):
    ...
    def build_main_ui(self, screen):
        ...
        def _dash_card(title, body_widget):
            card = Card(height=dp(70), spacing=dp(4), padding=[dp(12), dp(8), dp(12), dp(8)])
            card.add_widget(SmallLabel(text=title, color=MUTED, font_size=sp(11)))
        ...
        sections_card.add_widget(SmallLabel(text='Projects now include status, priority, pipeline stage, attachments, build history, archive support, and timestamped activity. File Generator saves GitHub APK build files to Downloads/Synapse by SHV/File Generator.'))
